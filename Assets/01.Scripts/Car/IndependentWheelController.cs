...
public class IndependentWheelController : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        // Front Left 바인딩
        inputActions.CarMap.WheelFL_Forward.performed += ctx => inputFL = 1f;
        inputActions.CarMap.WheelFL_Forward.canceled += ctx => inputFL = 0f;
        inputActions.CarMap.WheelFL_Backward.performed += ctx => inputFL = -1f;
        inputActions.CarMap.WheelFL_Backward.canceled += ctx => inputFL = 0f;

        // Front Right 바인딩
        inputActions.CarMap.WheelFR_Forward.performed += ctx => inputFR = 1f;
        inputActions.CarMap.WheelFR_Forward.canceled += ctx => inputFR = 0f;
        inputActions.CarMap.WheelFR_Backward.performed += ctx => inputFR = -1f;
        inputActions.CarMap.WheelFR_Backward.canceled += ctx => inputFR = 0f;

        // Rear Left 바인딩
        inputActions.CarMap.WheelRL_Forward.performed += ctx => inputRL = 1f;
        inputActions.CarMap.WheelRL_Forward.canceled += ctx => inputRL = 0f;
        inputActions.CarMap.WheelRL_Backward.performed += ctx => inputRL = -1f;
        inputActions.CarMap.WheelRL_Backward.canceled += ctx => inputRL = 0f;

        // Rear Right 바인딩
        inputActions.CarMap.WheelRR_Forward.performed += ctx => inputRR = 1f;
        inputActions.CarMap.WheelRR_Forward.canceled += ctx => inputRR = 0f;
        inputActions.CarMap.WheelRR_Backward.performed += ctx => inputRR = -1f;
        inputActions.CarMap.WheelRR_Backward.canceled += ctx => inputRR = 0f;
    }
